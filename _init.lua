------------------------------------------------------------------------------
-- Modify from https://github.com/nodemcu/nodemcu-firmware/blob/master/lua_examples/lfs/_init.lua
...
--[[
  ...
  module related initialisation in this. This example uses standard Lua features to
...
-- Load LFS img if scheduled.
if file.open('readImg', "r") then
  imageFileName = file.readline()
  file.remove('readImg')
  if not file.exists(imageFileName) then
    print('img file: "' .. imageFileName .. '" not exists.')
  else
    print('flash img: ' .. imageFileName)
    node.flashreload(imageFileName or 'lfs.img')
    return
  end
end
